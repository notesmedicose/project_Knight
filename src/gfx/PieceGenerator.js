...
export class PieceGenerator {
  constructor() {
    // Materials will be fully initialized after setTextures() is called
    this.textures = null;
    this._initBaseMaterials();
  }

  /**
   * Apply procedural textures to all piece materials
   * Called after textures are generated by ProceduralTextureGenerator
   */
  ...
  _initBaseMaterials() {
    ...
    // ─── White Body: Polished White Marble ───────────────────
    this.materials = {
      whiteBody: new THREE.MeshPhysicalMaterial({
        color: 0xf6f4ee,
        roughness: 0.15,
        metalness: 0.02,
        clearcoat: 0.9,
        clearcoatRoughness: 0.08,
        map: t?.marble?.map || null,
        bumpMap: t?.marble?.bumpMap || null,
        bumpScale: 0.03,
        envMapIntensity: 0.8,
      }),

      // ─── Gold Accents (White & Black Pieces) ───────────────
      whiteAccent: new THREE.MeshPhysicalMaterial({
        color: 0xdfb438,
        metalness: 0.92,
        roughness: 0.18,
        emissive: 0x3d2b00,
        ...
        map: t?.gold?.map || null,
        roughnessMap: t?.gold?.roughnessMap || null,
        metalnessMap: t?.gold?.metalnessMap || null,
        bumpMap: t?.gold?.bumpMap || null,
        bumpScale: 0.02,
        envMapIntensity: 1.5,
        clearcoat: 0.3,
      }),
      ...
      blackAccent: new THREE.MeshPhysicalMaterial({
        color: 0xdfb438,
        metalness: 0.92,
        roughness: 0.18,
        emissive: 0x3d2b00,
    ...
    // Ensure existing meshes get updated materials
    Object.keys(this.materials).forEach(key => {
      if (this.materials[key]) this.materials[key].needsUpdate = true;
    });
  }
